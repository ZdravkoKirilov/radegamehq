...
def with_nesting(entities=[]):
    def decorator(cls):
        class NestedSerializerBase(cls):

            def __init__(self, *args, **kwargs):
                cls.__init__(self, *args, **kwargs)
                self.nested_entities = entities
            ...
            def update_items(self, items: List[Union[DefaultDict, int]], entity_model: base,
            ...
            def remove_items(cls, items: List[DefaultDict], existing: QuerySet, entity_model: Model, m2m: Manager):
                ...
                item_ids = [item.id for item in items] if m2m is not None else [item['id'] for item in items if
                                                                                'id' in item]
                redundant = existing.exclude(pk__in=item_ids) if existing is not None else None
